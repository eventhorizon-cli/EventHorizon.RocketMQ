...
这里记录项目中不容易从 API 签名看出的设计取舍：为什么 gRPC 与经典 Remoting 是两套独立的客户端，
为什么 Consumer 的生命周期由 Host 管理，以及测试环境如何覆盖真正的 NameServer、Broker 与 Proxy
交互。这些文章面向需要评估、维护或扩展客户端的开发者；日常接入请先阅读[仓库总览](../../README.zh-CN.md)
和[可运行示例](../../samples/README.zh-CN.md)。
...
| 架构 | [依赖注入与生命周期](architecture/dependency-injection-and-lifetimes.md) | profile、键控服务、Consumer Engine 和消息处理器如何组成一个 Host？ |
...
- Docker Compose 手工环境位于
  [`test-environments/rocketmq`](../../test-environments/rocketmq)。

文档描述的是仓库当前实现，而不是 RocketMQ 所有版本、所有部署形态的通用兼容性承诺。涉及服务端能力、
topic 类型、ACL 或网络拓扑时，应同时阅读相应协议包 README 和对应 sample 的 README。
