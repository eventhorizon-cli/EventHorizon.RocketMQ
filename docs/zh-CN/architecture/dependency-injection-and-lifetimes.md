# 依赖注入与生命周期：一个 profile 是一个明确的客户端边界
...
Admin。它们可共享某些配置，却不能共享一个含糊的“客户端生命周期”：角色有不同的 options、后台任务
和身份标识。
...
独立的配置空间；`"orders"` 与 `"analytics"` 同时是 options 名称和键控服务的键。

核心规则如下：

- 每个 `(profile, role)` 只能注册一次，避免同一身份重复启动后台客户端。
- 对外角色接口按协议注册为 singleton；命名 profile 使用 keyed singleton，默认 profile 使用普通
  singleton。
- 每个 Consumer 角色在对应 builder 的组合根中创建一个私有 Engine；Engine 不注册为可跨角色共享的
  全局服务。
- Producer 与自动消费角色通过 `IHostedService` 随 Generic Host 启停。Admin 等没有后台循环的角色不
  需要独立 hosted service。
...
`AddRocketMQGrpc`/`AddRocketMQRemoting` 先写入 named options，并验证 endpoint、ACL 成对配置及时间参数。
...
注册阶段会为每个角色生成一个逻辑 client 名称。这样同一个 profile 中的 Producer 与 Consumer 即使连接同一
...
不应在多个 Consumer 之间共享。这样停止某个角色不会意外停止另一个角色的循环，选项、group 和 handler
也不会相互覆盖。

### 3. Host 负责启动和停止

`AddGrpcProducer`、`AddGrpcPushConsumer`、`AddRemotingProducer`、`AddRemotingPushConsumer` 等自动运行
角色会注册对应 hosted service。Host 启动时调用角色的 `StartAsync`，停止时调用 `StopAsync`，随后由
DI 容器负责释放资源。
...
### 4. 键控 profile 保持多集群选择显式

同一 Host 需要多个同类型角色时，使用命名 profile 并在消费方选择键：
...
键不是 Broker 的功能，也不是消息路由标签；它只是在本地 DI 容器中选择一个已配置的客户端角色。默认
profile 使用普通构造函数注入，不需要 `FromKeyedServices`。

### 5. typed handler 的 scope 以“单次处理”为边界

Push 和 LitePush 可接收委托，也可注册实现 `IGrpcPushMessageHandler` 或
`IRemotingPushMessageHandler` 的类型。typed handler 的生命周期由调用方选择：

| handler 生命周期 | 实例与 scope 行为 |
| --- | --- |
| `Singleton` | 直接复用一个 handler；它只能依赖 singleton 或安全的工厂/抽象。 |
| `Scoped` | 每次消息处理创建一个异步 scope，并从该 scope 解析 handler。 |
| `Transient` | 同样为每次处理创建异步 scope，再取得新的 handler。 |
...
**一个 profile 的一个角色只能有一个实例。** 需要同协议的多个 Producer 或 Consumer 时，应创建不同的
命名 profile，而不是重复对一个 builder 调用相同的 `Add...` 方法。
