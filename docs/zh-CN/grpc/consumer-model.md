...
gRPC 客户端连接的是 RocketMQ Proxy。它提供的 Consumer API 应反映 Proxy 可提供的工作流，而不是复制
经典客户端的全部名称。本项目因此把 gRPC 消费建模为三种明确角色：Simple、Push 和 LitePush。
...
位点服务端 RPC；当目标 Proxy 没有可用实现时，保留一个看似对等的 public API 只会让应用在部署后失败。

## 决策

gRPC 包公开以下 Consumer 角色：
...
不提供 `IGrpcPullConsumer`。这不是未完成的 sample，也不是配置开关；该 public role 已从仓库移除。
...
`PullMessage`、`GetOffset`、`UpdateOffset` 和 `QueryOffset`，因为它是上游 wire 定义的一部分；这不等于本包
支持 gRPC Pull。内部
...
1. 使用 `Subscribe` 或 `SubscribeAsync` 建立 topic/过滤表达式。
...
FIFO 分发时，客户端为同一消息组维持顺序尾链并阻塞同组后续消息；它是应用处理顺序的约束，不应被理解为
跨 group、跨队列或跨消费者的全局顺序保证。

通过 `AddGrpcPushConsumer<THandler>(ServiceLifetime.Scoped, ...)` 注册 typed handler 时，每次处理会创建
...
- 目标 Broker 支持 Lite 消息，并已为 Lite 配置所需能力；
- consumer group 配置了 bind topic；
- Proxy 实现 `SyncLiteSubscription`；
- 应用使用 `SubscribeLiteAsync`/`UnsubscribeLiteAsync` 管理 LiteTopic，而不是将它当作普通 topic。

LitePush 不是 gRPC Pull 的替代名称。它仍然是客户端发起长轮询的自动分发模型，额外解决的是 Lite 订阅
同步和 offset 选项。

### 生命周期与订阅变更

当通过 Generic Host 注册角色时，对应 hosted service 会启动和停止 Consumer。`SubscribeAsync`、
`UnsubscribeAsync` 和 Lite 订阅变更会通知接收循环重新协调；它们不是重新注册一个 DI 服务。
...
序列化配置修复的客户端细节。对显式 queue/offset 工作流，经典 Remoting Pull 是当前明确的协议选择。

**Server capability 是部署前提。** Lite、死信、过滤、FIFO、事务和最小长轮询参数均可能依赖 Broker/Proxy
版本与配置。请以部署环境和[gRPC 包指南](../../../src/EventHorizon.RocketMQ.Grpc/README.zh-CN.md)为准，并从
