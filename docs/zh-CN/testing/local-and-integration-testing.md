...
Remoting Producer 心跳的 UT 使用可控时钟和隔离的协议协作者，验证目标发现、空闲续期、故障隔离、地址替换和
停止后重启的顺序。Docker IT 直接读取 Broker 的 Producer 注册记录与最近更新时间，确认没有后续消息发送时
仍会注册和续期，并检查停止后的注销。恢复场景通过 Producer 自己的连接主动注销成员记录，再验证下一次周期
心跳能重新建立记录，无须依赖不稳定的网络故障注入。
事务 IT 还会让本地执行器返回 `Unknown`，等待真实 Broker 调用已注册的事务检查器，再通过回查提交事务并确认
消息可以消费。该用例沿用已发布 Broker 的正常回查周期，不修改共享 fixture 的事务配置。
...
- 一次 `mqadmin consumerProgress -g` 已经会返回整个 Consumer Group 的进度快照。需要断言多个 Queue 时，应解析并
