...
Use the [Remoting Producer](../../../remoting/GenericHost/Producer/README.md) instead when the application must use classic-only
...
To exercise the independent keyed `audit` registration, use its endpoint:
...
Each successful request returns HTTP 200 with a `messageId`, the fixed `topic`, and the RocketMQ storage
`offset`. That receipt confirms service acceptance, not Consumer completion. Swagger exposes the same operations at
...
creation is disabled. The sample keeps connection settings in `appsettings.json` and declares its fixed Producer
behavior next to each registration.
