...
[Producer sample](../Producer/README.md) or another compatible producer to publish matching messages after this host
has started. The sample logs each received message, then calls `AckAsync`. It continues long-polling until Ctrl+C;
the Generic Host then stops both the sample receive loop and the SDK role.
