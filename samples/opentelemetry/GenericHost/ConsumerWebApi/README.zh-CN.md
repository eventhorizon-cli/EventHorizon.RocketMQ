...
[OpenTelemetry 总览](../../README.zh-CN.md) | [Generic Host 示例](../README.zh-CN.md) | [English](README.md)
...
默认配置需要 Docker Compose 以及由 `global.json` 选择的 .NET SDK。它连接仓库提供的 RocketMQ Proxy
`localhost:8081`、NameServer `localhost:9876` 和 OTLP/gRPC collector `http://127.0.0.1:4317`。RocketMQ
环境的 initializer 会在 `up --wait` 返回前创建 `eventhorizon-test-topic`。

在仓库根目录启动共享环境，再运行 Consumer 集成：
...
使用默认 launch profile 时，Swagger 位于 [http://localhost:5242/swagger](http://localhost:5242/swagger)。
`GET /health` 只验证 Web Host 正在响应。`GET /consumers` 会列出配置的 topic 和两个 Consumer 角色类型，并不探测
Broker 连通性。Host 会在应用启动时启动两个已注册的 Consumer 角色，并在 Host 关闭时停止它们；因此应用代码不应
调用这些角色的生命周期方法。

在另一个终端启动 [Producer 集成](../ProducerWebApi/README.zh-CN.md)。其 Swagger UI 位于
`http://localhost:5241/swagger`，可通过 `POST /messages/grpc` 或 `POST /messages/remoting` 发送消息。Consumer
Host 启动后发送的消息会由 gRPC 和 Remoting Consumer Group 分别处理，并产生对应的控制台日志。Exporter 刷新后，
可在 [OpenTelemetry 总览](../../README.zh-CN.md)说明的 Consumer dashboard 中查看 receive、process 和 settlement
telemetry。
