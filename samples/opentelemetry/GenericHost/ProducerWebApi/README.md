...
From the repository root, start the shared RocketMQ and Grafana OTEL LGTM environments, then run the Producer
integration:
...
The host listens at `http://localhost:5241`; open `http://localhost:5241/swagger`, or send a JSON body containing a
non-empty `message` to `POST /messages/grpc` or `POST /messages/remoting`. A successful request returns HTTP `200`
and a protocol-specific send receipt. That receipt confirms the send operation, not Consumer processing; use the
[Consumer integration](../ConsumerWebApi/README.md) to observe receive, process, and settlement signals, and the
[OpenTelemetry overview](../README.md) for the complete requests, dashboards, and local topology.
