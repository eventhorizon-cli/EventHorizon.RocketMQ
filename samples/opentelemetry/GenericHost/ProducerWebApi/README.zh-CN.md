...
在仓库根目录启动共享 RocketMQ 与 Grafana OTEL LGTM 环境，再运行 Producer 集成：
...
host 监听在 `http://localhost:5241`；可打开 `http://localhost:5241/swagger`，或向 `POST /messages/grpc` 或
`POST /messages/remoting` 发送包含非空 `message` 的 JSON 请求。发送成功会返回 HTTP `200` 及协议专用的发送回执。
该回执只确认发送操作，不代表 Consumer 已完成处理；使用 [Consumer 集成](../ConsumerWebApi/README.zh-CN.md)观察接收、处理和
结算信号，完整请求、dashboard 与本地拓扑请参阅 [OpenTelemetry 总览](../README.zh-CN.md)。
