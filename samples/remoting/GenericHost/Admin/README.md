...
After the API starts, discover the topic queues from a second terminal:

```shell
curl http://localhost:5185/topics/eventhorizon-test-topic/queues
```

The response is a non-empty JSON array of `brokerName` and `queueId` values. Substitute one returned pair into a
per-queue request:
...
Swagger lists every read-only endpoint at `http://localhost:5185/swagger`. To view a particular stored message, send
one with the [Remoting Producer](../Producer/README.md), then substitute its `offsetMessageId` into
`/topics/eventhorizon-test-topic/messages/<offset-message-id>`. The Admin API does not create or modify RocketMQ
resources.

For an external cluster, configure `RocketMQ:Remoting:NamesrvAddr`, create the topic when required, and grant the
route-query and read permissions required by that cluster.
