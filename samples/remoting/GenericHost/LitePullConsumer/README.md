...
route-query and consume permissions. Start this host before using the Remoting Producer sample to publish messages.
It logs each message from an assigned queue, then commits the advanced local positions after sample processing.
Press Ctrl+C to stop the host and its SDK role.
