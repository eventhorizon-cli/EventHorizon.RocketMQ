...
dotnet run --project samples/remoting/GenericHost/PopConsumer/EventHorizon.RocketMQ.Samples.Remoting.PopConsumer.csproj
```
...
Start this host before using the [Remoting Producer sample](../Producer/README.md) to publish a message. The loop logs
the selected physical queue and message, then acknowledges its receipt after the sample processing completes. Press
Ctrl+C to stop the host and its SDK role.
