...
暴露的是 RocketMQ 5 Proxy，且不需要 classic 专属操作，应改用 [gRPC Producer](../../../grpc/GenericHost/Producer/README.zh-CN.md)。
...
示例的 HTTP 响应还会返回 `OffsetMessageId`。将这个物理 ID 传给
[Remoting Admin 示例](../Admin/README.zh-CN.md)即可读取已存储消息；它不能与 `MessageId` 互换。
...
将 `/messages` 替换为 `/clients/audit/messages`，即可通过独立的具名 `audit` keyed registration 发送：
...
在仓库提供的正常环境中，每个请求都会返回 HTTP 200，`status` 为 `SendOk`，并包含选中的 Broker 队列、位点及
`offsetMessageId`。保留该物理 ID，可通过 [Admin 示例](../Admin/README.zh-CN.md)检查存储消息。Swagger 在
`http://localhost:5184/swagger` 提供相同操作。

连接外部集群时，配置 `RocketMQ:Remoting:NamesrvAddr`，确保每个 Broker 公布端点都可访问，并在禁用自动创建资源时
预先创建 Topic。示例只在 `appsettings.json` 中保留连接配置，固定的 Producer 行为直接写在各自的注册代码旁。
