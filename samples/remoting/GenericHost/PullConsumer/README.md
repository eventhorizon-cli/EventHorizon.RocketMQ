...
dotnet run --project samples/remoting/GenericHost/PullConsumer/EventHorizon.RocketMQ.Samples.Remoting.PullConsumer.csproj
```
...
consumer starts. The long-running host logs each received message with its physical queue and offset, then persists
`NextOffset` after that sample processing completes. Press Ctrl+C to stop the host and its SDK role.
