...
此 Generic Host 通过 `AddRemotingPushConsumer<PushConsumerMessageHandler>` 注册 scoped 的
`PushConsumerMessageHandler`，并通过 `IRemotingPushConsumer` 处理消息。这是带有自动分配、长轮询、分发、重试和
位点处理的高级经典 Remoting Consumer。
...
| `RocketMQ:PushConsumer:MaxConcurrency` | `4` | 可并发运行的最大消息处理器调用数。 |
| `RocketMQ:PushConsumer:BatchSize` | `32` | 每个长轮询请求的最大消息数。 |
| `RocketMQ:PushConsumer:LongPollingTimeout` | `00:00:05` | 空长轮询在 Broker 上的最大等待时间。 |
| `RocketMQ:PushConsumer:RetryDelay` | `00:00:01` | 接收或处理器结果失败后的重试等待时间。 |
| `Sample:Subscriptions[0]:Topic` | `rocketmq-dotnet-manual` | 默认订阅的普通 topic。 |
| `Sample:Subscriptions[0]:Filter` | `*` | 订阅过滤条件。 |

处理器记录消息并返回 `ConsumeResult.Success`。带类型的注册使处理器可以按 scoped 生命周期从应用服务提供程序中获取
`ILogger<PushConsumerMessageHandler>`。

## Broker 和网络前置条件

- 使用经典 Remoting NameServer 和 Broker。客户端通过 `NamesrvAddr` 发现路由，随后向公布的 Broker 建立长轮询
  连接，因此这两跳都必须可访问。
...
- 随附的[本地 RocketMQ 环境](../../../test-environments/rocketmq/README.md)适合在宿主机运行此普通 topic Remoting
  Consumer。在仓库根目录准备资源：
...
随附栈会公布宿主机可访问的 Broker 地址。位于其他容器中的进程必须使用网络可访问的 NameServer 和 Broker 地址，
...
- 虽然名称中带有 Push，但它不是 Broker 向应用进行协议级推送。客户端负责队列分配，并反复发起长轮询 pull 请求，
  然后将收到的消息分发给处理器。
- `ConsumeResult.Success` 表示处理成功并推进消费。短暂失败时应返回 retry 结果；Broker 的重试和死信行为以 consumer
  group 为基础，因此处理器必须幂等并能容忍重复投递。
