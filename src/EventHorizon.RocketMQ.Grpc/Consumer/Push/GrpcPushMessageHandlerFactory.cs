...
internal static class GrpcPushMessageHandlerFactory
{
        ...
        where TMessageHandler : class, IGrpcPushMessageHandler
    {
        ...
        switch (lifetime)
        {
            case ServiceLifetime.Singleton:
                services.AddKeyedSingleton<IGrpcPushMessageHandler, TMessageHandler>(roleKey);
                return;
            case ServiceLifetime.Scoped:
                services.AddKeyedScoped<IGrpcPushMessageHandler, TMessageHandler>(roleKey);
                return;
            case ServiceLifetime.Transient:
                services.AddKeyedTransient<IGrpcPushMessageHandler, TMessageHandler>(roleKey);
                return;
            default:
    ...
    internal static Func<GrpcMessageView, CancellationToken, ValueTask<ConsumeResult>> Create(
        ...
        ServiceLifetime lifetime)
    {
        ...
        {
            ServiceLifetime.Singleton => provider.GetRequiredKeyedService<IGrpcPushMessageHandler>(roleKey).HandleAsync,
    ...
    private static Func<GrpcMessageView, CancellationToken, ValueTask<ConsumeResult>> CreateScopedHandler(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var handler = scope.ServiceProvider.GetRequiredKeyedService<IGrpcPushMessageHandler>(roleKey);
