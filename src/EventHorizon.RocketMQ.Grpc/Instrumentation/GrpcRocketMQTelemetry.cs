...
internal sealed class GrpcRocketMQTelemetry : IGrpcRocketMQTelemetry
{
    ...
    private GrpcRocketMQTelemetry()
    {
    }

    public GrpcRocketMQTelemetry(IMeterFactory meterFactory)
    {
        ArgumentNullException.ThrowIfNull(meterFactory);
        var meter = meterFactory.Create(new MeterOptions(GrpcRocketMQInstrumentation.MeterName));
    ...
    internal static GrpcRocketMQTelemetry Disabled { get; } = new();

    public IGrpcRocketMQTelemetryOperation StartSend(string topic, int messageCount, long bodySize) =>
        Start("send", "send", ActivityKind.Producer, topic, null, messageCount, bodySize, null, null, null, null);

    public IGrpcRocketMQTelemetryOperation StartReceive(
        string topic,
    ...
    public IGrpcRocketMQTelemetryOperation StartProcess(
        string topic,
        string consumerGroup,
        string messageId,
        int queueId,
    ...
    public IGrpcRocketMQTelemetryOperation StartSettle(
    ...
    private IGrpcRocketMQTelemetryOperation Start(
        string operationName,
        ...
        bool createActivity = true)
    {
        ...
        var recordActivity = createActivity && GrpcRocketMQInstrumentation.ActivitySource.HasListeners();
        ...
        if (recordActivity)
        {
            ...
            activity = GrpcRocketMQInstrumentation.ActivitySource.StartActivity(
                $"{operationName} {topic}",
                kind,
                parent,
                tags,
    ...
    internal sealed class Operation : IGrpcRocketMQTelemetryOperation
    {
