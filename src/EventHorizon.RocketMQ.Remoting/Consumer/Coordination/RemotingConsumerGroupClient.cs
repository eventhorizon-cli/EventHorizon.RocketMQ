...
internal sealed class RemotingConsumerGroupClient
{
        ...
        foreach (var broker in DistinctBrokers(brokers))
        {
            try
            {
                var response = await _remotingClient.InvokeAsync(
                    ...
                    new RemotingCommand(RequestCode.HeartBeat) { Body = heartbeat },
        ...
        CancellationToken cancellationToken)
    {
        ...
        return LegacyConsumerListProtocol.Decode(response.Body);
    }
