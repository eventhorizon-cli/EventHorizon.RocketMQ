...
internal sealed record PushReceiveTarget(
    RemotingPushAssignment Assignment,
    RemotingBrokerEndpoint Broker,
    FilterExpression Filter)
{
    public RemotingConsumerQueue? Queue => Assignment.QueueId >= 0
        ? new RemotingConsumerQueue(Assignment.Topic, Assignment.BrokerName, Assignment.QueueId)
        : null;
}
