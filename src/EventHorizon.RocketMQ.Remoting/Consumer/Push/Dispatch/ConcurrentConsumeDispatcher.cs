...
namespace EventHorizon.RocketMQ.Remoting.Consumer.Push.Dispatch;

internal sealed class ConcurrentConsumeDispatcher
{
    private readonly Channel<ProcessQueue> _readyProcessQueues;
    private readonly int _consumeMessageBatchSize;
    private readonly Func<ConsumeRequest, CancellationToken, ValueTask> _consumeRequest;
    private readonly Action<int> _releaseDeliverySlots;
    private readonly CancellationToken _stopping;
    private readonly ILogger _logger;

    public ConcurrentConsumeDispatcher(
        int maxConcurrency,
        int consumeMessageBatchSize,
        Func<ConsumeRequest, CancellationToken, ValueTask> consumeRequest,
        Action<int> releaseDeliverySlots,
        ...
        ILogger logger)
    {
        ...
        ArgumentNullException.ThrowIfNull(releaseDeliverySlots);
        ArgumentNullException.ThrowIfNull(logger);

        _consumeMessageBatchSize = consumeMessageBatchSize;
        _consumeRequest = consumeRequest;
        _releaseDeliverySlots = releaseDeliverySlots;
        _stopping = stopping;
        _logger = logger;
        _readyProcessQueues = Channel.CreateUnbounded<ProcessQueue>(new UnboundedChannelOptions
        {
    ...
    public void NotifyReady(ProcessQueue processQueue)
    {
    ...
    private async Task RunConsumeLoopAsync()
    {
        try
        {
            await foreach (var processQueue in _readyProcessQueues.Reader.ReadAllAsync(_stopping).ConfigureAwait(false))
            {
                ...
                _releaseDeliverySlots(request.ReservedDeliverySlots);
