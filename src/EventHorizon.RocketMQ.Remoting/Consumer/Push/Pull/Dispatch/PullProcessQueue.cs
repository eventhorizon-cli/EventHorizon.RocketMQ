...
// role of RocketMQ's PullProcessQueue; it is not a Broker-side physical queue.
internal sealed class PullProcessQueue(RemotingConsumerQueue messageQueue, CancellationToken queueCancellation)
{
    ...
    private int _activeSettlementCount;
    private TaskCompletionSource? _settlementsDrained;
    private bool _initialized;
    private bool _readyQueued;
    private bool _admissionBlocked;
    private bool _dropped;

    public RemotingConsumerQueue MessageQueue { get; } = messageQueue;
    ...
    public bool TryCreateConsumeRequest(int batchSize, [NotNullWhen(true)] out PullConsumeRequest? request)
    {
        if (batchSize <= 0)
        {
        ...
        lock (_gate)
        {
            ...
            foreach (var entry in selected)
            {
                ...
                if (entry.HasReservedDeliverySlot)
                {
                    ...
                    reservedBodyBytes = checked(reservedBodyBytes + entry.Message.Body.Length);
                }
            }

            request = new PullConsumeRequest(
                this,
                selected,
                new PullCacheReservation(reservedMessageCount, reservedBodyBytes),
                settlementRetry);
            return true;
        }
    }

    public bool TryStartConsumeRequest(PullConsumeRequest request)
    {
        ArgumentNullException.ThrowIfNull(request);
        if (!ReferenceEquals(request.PullProcessQueue, this))
        {
    ...
    public bool CompleteConsumeRequest(PullConsumeRequest request, IReadOnlyList<bool> completed)
    {
        ArgumentNullException.ThrowIfNull(request);
        ...
        bool dropped;
        TaskCompletionSource? settlementsDrained;
        lock (_gate)
        {
            settlementsDrained = CompleteSettlementsCore(request.Entries);
            dropped = _dropped;
            if (dropped)
            {
                hasPending = false;
        ...
        settlementsDrained?.TrySetResult();
        if (dropped)
        {
            return false;
        }
    ...
    public bool AbandonConsumeRequest(PullConsumeRequest request)
    {
        ArgumentNullException.ThrowIfNull(request);
        if (!ReferenceEquals(request.PullProcessQueue, this))
        {
    ...
    public bool TryBeginSettlement(
        ...
        int delayLevel,
        out PullCacheReservation reservation)
    {
        ...
        lock (_gate)
        {
            if (_dropped ||
                ...
                entry.SettlementInProgress)
            {
                reservation = default;
                return false;
            }

            if (_activeSettlementCount == 0)
            {
                _settlementsDrained = new TaskCompletionSource(
                    TaskCreationOptions.RunContinuationsAsynchronously);
            }

            _activeSettlementCount++;
            entry.SettlementInProgress = true;
            entry.PendingSettlementResult = result;
            entry.PendingSettlementDelayLevel = delayLevel;
            _admissionBlocked = true;
            reservation = ReleaseAdmissionBlockedDeliverySlotsCore();
            return true;
        }
    }
    ...
    public PullCacheReservation ReleaseAdmissionBlockedDeliverySlots()
    {
        lock (_gate)
        {
    ...
    public Task WaitForActiveSettlementsAsync()
    {
        lock (_gate)
        {
            return _settlementsDrained?.Task ?? Task.CompletedTask;
        }
    }

    public PullCacheReservation MarkDropped(Action<MessageEntry>? dropFifoOrder = null)
    {
        var reservedMessageCount = 0;
        var reservedBodyBytes = 0;
        ...
        Signal(_admissionAvailable);
        Signal(_stateChanged);
        return new PullCacheReservation(reservedMessageCount, reservedBodyBytes);
    }
    ...
    private TaskCompletionSource? CompleteSettlementsCore(IEnumerable<MessageEntry> entries)
    {
        foreach (var entry in entries)
        {
            if (!entry.SettlementInProgress)
            {
                continue;
            }

            entry.SettlementInProgress = false;
            _activeSettlementCount--;
        }

        if (_activeSettlementCount < 0)
        {
            throw new InvalidOperationException("The active PULL settlement count became negative.");
        }
        ...
        var drained = _settlementsDrained;
        _settlementsDrained = null;
        return drained;
    }
    ...
    private PullCacheReservation ReleaseAdmissionBlockedDeliverySlotsCore()
    {
        ...
        var releasedMessageCount = 0;
        var releasedBodyBytes = 0;
        foreach (var entry in _messages.Values)
        {
            if (!entry.HasReservedDeliverySlot)
            {
            ...
            releasedBodyBytes = checked(releasedBodyBytes + entry.Message.Body.Length);
        }

        return new PullCacheReservation(releasedMessageCount, releasedBodyBytes);
    }
    ...
    internal sealed class MessageEntry(RemotingMessageView message)
    {
        ...
        public bool SettlementInProgress { get; set; }
