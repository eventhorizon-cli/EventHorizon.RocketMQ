...
public sealed class RemotingConsumerGroupClientTests
{
    [Fact]
    public void Constructor_NamespacedGroupAndConfiguredClientIdentity_Builds()
    {
        ...
        Assert.Equal("10.0.0.4@consumer-instance@unit-a", client.ClientId);
        Assert.Equal("tenant-a%orders-consumer", client.ConsumerGroup);
    }
    ...
    public async Task SendHeartbeatsAsync_BrokerFailure_ContinuesAndDeduplicatesAddresses()
    {
        ...
        await client.SendHeartbeatsAsync(
            [
    ...
    public async Task GetConsumerIdsAsync_NamespacedGroupAndBrokerName_UsesIdentity()
    {
        ...
        var broker = new RemotingBrokerEndpoint("broker-a", "127.0.0.1:10911");

        var consumerIds = await client.GetConsumerIdsAsync(queue, broker, cancellationToken);
    ...
    private static RemotingConsumerGroupClient CreateClient(IRemotingClient remoting) =>
        new(
            remoting,
