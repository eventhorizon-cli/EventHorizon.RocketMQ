...
namespace EventHorizon.RocketMQ.Remoting.Tests.Consumer.Push.Offset;

public sealed class LegacyResetOffsetDecoderTests
{
    [Fact]
    public void Decode_MissingOffsetTable_ReturnsEmpty()
    {
    ...
    [Fact]
    public void Decode_StringKeysAndNumericStrings_Decodes()
    {
        var offsets = Decode(
            """
    ...
    [Fact]
    public void Decode_FastJsonEmptyMapAndEscapedStringValues_DecodesValues()
    {
    ...
    [InlineData("{\"offsetTable\":{{\"topic\":\"orders\",\"brokerName\":\"broker-a\",\"queueId\":0}:\"7}}")]
    public void Decode_InvalidPayloads_Rejects(string json)
    {
        Assert.Throws<InvalidDataException>(() => Decode(json));
    }

    [Fact]
    public void Decode_InvalidUtf8_Rejects()
    {
