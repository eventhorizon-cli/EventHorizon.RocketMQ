...
public sealed class ConcurrentPullConsumeDispatcherTests
{
    [Fact]
    public async Task ProcessQueues_OneConsumeLoop_RotatesActiveQueues()
    {
        ...
        var consumed = 0;
        var dispatcher = new ConcurrentPullConsumeDispatcher(
            maxConcurrency: 1,
            consumeMessageBatchSize: 1,
            (request, _) =>
            {
                order.Enqueue(request.PullProcessQueue.MessageQueue.QueueId);
                request.PullProcessQueue.CompleteConsumeRequest(request, [true]);
    ...
    [Fact]
    public async Task ConsumeCallback_DroppedRequest_DoesNotInvokeCallback()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        using var stopping = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
    ...
    private static PullProcessQueue CreateProcessQueue(
        int queueId,
        int messageCount,
        CancellationToken cancellationToken)
    {
        var queue = new RemotingConsumerQueue("orders", "broker-a", queueId);
        var processQueue = new PullProcessQueue(queue, cancellationToken);
